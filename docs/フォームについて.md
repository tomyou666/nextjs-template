...
Next.jsではServer Actionsという、SSR、SSGなどのレンダリング済みのコンポーネントでもHTMLのformの機能を利用した、サーバーとのやり取りを動的に行うことができる機能があります。

```tsx
// src/lib/backend/actions.ts
'use server'
...
Zodのスキーマは`lib/share/schemas.ts`に定義します。このファイルには、フロントエンドとバックエンド両方で使用するバリデーションスキーマを集約します。

```tsx
// lib/share/schemas.ts
import { z } from 'zod'
...
## Conformによるフォーム管理(クライアントサイド)

Conformを使用してフォームの状態管理とバリデーションを行います。

```tsx
// src/app/dashboard/forms/TextInputSample.tsx
'use client'
